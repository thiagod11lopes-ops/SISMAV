...
const backupPath = path.join(__dirname, '..', '..', '..', 'Downloads', 'SISMAV_Backup_2026-02-06T14-16-02.json');
...
function obterDiaSemana(dateStr) {
    if (!dateStr) return '';
    try {
        const date = new Date(dateStr + 'T00:00:00');
...
<body>
    <div class="header-info">
        ...
        <p style="text-align: center; color: #4b5563; font-size: 14px; margin-bottom: 20px;">
...
`;
    if (viaturasNaOficina.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="5" style="text-align: center; color: #6b7280;">Nenhuma viatura registrada na oficina</td>
                </tr>
        `;
    }
    html += `
...
`;
    if (viaturasFinalizadas.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="5" style="text-align: center; color: #6b7280;">Nenhuma viatura finalizada nos últimos 10 dias</td>
                </tr>
        `;
    }
    html += `
...
`;
    if (fainasPendentes.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="3" style="text-align: center; color: #6b7280;">Nenhuma atividade pendente registrada</td>
                </tr>
        `;
    }
    html += `
...
`;
    if (fainasAndamento.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="3" style="text-align: center; color: #6b7280;">Nenhuma atividade em andamento registrada</td>
                </tr>
        `;
    }
    html += `
...
`;
    if (fainasFinalizadas.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="3" style="text-align: center; color: #6b7280;">Nenhuma atividade finalizada registrada</td>
                </tr>
        `;
    }
    html += `
...
`;
    if (servicosNaoAprovados.length > 0) {
        ...
        html += `
                <tr>
                    <td colspan="5" style="text-align: center; color: #6b7280;">Nenhum serviço não aprovado encontrado</td>
                </tr>
        `;
    }
    html += `
...
async function gerarPDF() {
    try {
        ...
        const htmlPath = path.join(__dirname, 'relatorio-temp.html');
        fs.writeFileSync(htmlPath, html, 'utf8');
        
        console.log('HTML gerado. Agora vamos converter para PDF usando puppeteer...');
        console.log('Por favor, instale o puppeteer executando: npm install puppeteer');
        console.log('\nOu use o HTML gerado em:', htmlPath);
        console.log('\nPara gerar o PDF, execute:');
        console.log('node gerar-relatorio-pdf-completo.js');
...
if (require.main === module) {
    gerarPDF().then(htmlPath => {
        console.log('\n✅ HTML do relatório gerado com sucesso!');
